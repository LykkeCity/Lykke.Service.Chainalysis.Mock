...
namespace Lykke.Service.ChainalysisMock.Core.Services
{
    public interface IChainalysisMockService
    {
        ...
        Task ImportUserAsync(string token, IUserImport map);
        Task<IUserDetails> GetUserAsync(string token, string userId);
        Task UpdateUserCommentAsync(string token, IUserComment userComment);
        ...
        Task<IUserWindrowAddressInfo> GetUserWithdrawAsync(string token, string userId, int? limit, int? offset);
        Task<IWindrowAddressInfo> AddAddressWithdrawAsync(string token, string userId, IAddressImport map);
        Task<object> DeleteWithdrawalAsync(string token, string userId, string address);
        Task<IUserTransactionInfo> GetUserOutputsSendsAsync(string token, string userId, TransactionStatus? map, int? limit, int? offset);
